...
void app_main()
{
    ...
    DEV_I2C_Init();
    // Initialize the external IO
    IO_EXTENSION_Init();
    vTaskDelay(10); // Wait for the external IO to be ready to receive information
    IO_EXTENSION_Output(IO_EXTENSION_IO_5, 1);  // Select communication interface: 0 for USB, 1 for CAN
    vTaskDelay(10); // Delay to ensure the interface is properly selected
